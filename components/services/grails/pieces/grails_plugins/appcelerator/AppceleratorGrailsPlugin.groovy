...
class AppceleratorGrailsPlugin {
    ...
    static def getAdapters = { message ->
        ...
        adapters.each { adapter ->
            ...
            if (adapter.requestType == message.type &&
                (version == null ||
                 version == "" ||
...
class ServiceAdapter {
    ...
    def call_service = { request, response ->
...
class Message {
    ...
    def makeResponse = {adapter ->
        if (adapter.responseType == null) {
            return null
        } else {
            return new Message (
                type: adapter.responseType,
    ...
    static def messagesFromRequest = { request ->
        def messages = []
        def timestamp = request.JSON.timestamp
    ...
    static def responseFromMessages = { messages, contentType ->
        def out = [:]
        out["version"] = "1.0"
        out["timestamp"] = System.currentTimeMillis()
        out["messages"] = []

        messages.each { response ->
            def out_mes = [:]
