...
# slight modifications by Jeff Haynie <jhaynie@gmail> for Appcelerator which are licensed
...
module Appcelerator
  ...
  #      puts "#{PACKAGE}: No UUID state file found, attempting to create one for you:"
        ...
        if addresses.empty?
          $stderr.puts "Could not find any IEEE 802 NIC MAC addresses for this machine."
          $stderr.puts "You need to create the uuid.state file manually."
        else
  ...
  #          file = File.basename(file) == 'lib' ? file = File.join(file, '..', STATE_FILE) : file = File.join(file, STATE_FILE)
        ...
        begin
          ...
          # Initialized.
          if @@logger
            @@logger.debug format(INFO_INITIALIZED, @@sequence, @@mac_addr)
          else
