...
module Appcelerator
  module Dispatcher
    ...
    #
    def self.dispatch_request(request, content_type, session, session_id, controller)
      message_queue = []
      extract_messages(request, content_type, session) do |in_message|
      ...
      serialize(message_queue, session_id, response, type)
      response.flush
      response.rewind
      response.read
    end
    ...
    #
    def self.dispatch_message(request, session, message_type, params, request_id, scope, controller, type = 'xml')
      ...
      response = StringIO.new
      serialize(message_queue, session.session_id, response, type)
      response.flush
      response.rewind
      response.read
    end
    
    def self.extract_messages(request, content_type, session)
    ...
    class Message
      attr_accessor :request, :session, :message_type, :params, :request_id, :response, :response_type, :scope, :controller
