...
/**
 * This code was ported by Josh Watts, Blue Violin and contributed to Appcelerator.
 * http://blue-violin.com
 */
...
'/' ; }
        ...
        if ( $handle = opendir ( $path ) )
        {
                while ( false !== ( $file = readdir ( $handle ) ) )
                {
                        ...
                        {
                                $file = $path . $file ;
                                if ( ! is_dir ( $file ) ) { if ( $mode != "DIRS" ) { $dirlist[] =
$file ; } }
...
{
        $matches = array();
        preg_match("/@Service[\s]*\([\s]*request[\s]*=(.*)?[\s]*[,]{0,1}
[\s]*(response[\s]*=[\s]*(.*)?)+[,]{0,1}(version[\s]*=[\s]*([0-9]+[.]
{0,1}[0-9]*)){0,1}\)/U", $str, $matches);
        $array = array();
...
class ServiceAdapter
{
        ...
        function ServiceAdapater(&$service, &$method, &$metadata)
        {
...
$dom = domxml_open_mem($post);
$docElement = $dom->document_element();
$nodes = $docElement->child_nodes();

$responseDom = domxml_new_doc("1.0");
$responseElement = $responseDom->create_element('messages');
$responseElement->set_attribute('version', '1.0');

if (!is_null($sessionid))
{
        $responseElement->set_attribute('sessionid', $sessionid);
}

$responseDom->append_child($responseElement);

function toXML ($dom, $response)
{
        $element = $dom->create_element('message');
        $element->set_attribute('type', $response['type']);
        $element->set_attribute('requestid', $response['requestid']);
        $element->set_attribute('datatype', 'JSON');
        $element->set_attribute('direction', 'OUTGOING');
        $element->set_attribute('timestamp', time());
        $cdata = $dom->create_cdata_section(json_encode($response['data']));
...
foreach ($nodes as $node)
{
        ...
        if ($node->node_type() == XML_ELEMENT_NODE)
        {
                ...
                {
                        $cdata = $cdata . $child->node_value();
                }
                $type = $node->get_attribute('type');
                if (array_key_exists($type,$services))
                {
                        $request = array();
                        $request['scope']  = $node->get_attribute('scope');
                        $request['version'] = $node->get_attribute('version');
                        $request['data'] = json_decode($cdata,true);
                        $request['requestid'] = $node->get_attribute('requestid');
                        $array = $services[$type];
