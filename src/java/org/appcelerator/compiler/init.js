/**
 * Original source code was adapted from:
 *
 ...
 *
 *
 * Modifications made by Appcelerator, Inc. 
 * Copyright 2007 Appcelerator, Inc., under the MIT License
 */
...
(function(){
	...
	function mapNode(node)
	{
		if (node.nodeType == 1)
		{
			...
			obj_node_map[String(id)]=makeNode(node);
		}
	}
	...
	DOMElement.prototype = extend( new DOMNode(), {
		...
		get attributes(){
			...
			{
				attr[i] = {};
				attr[i].name = String(attrs.item(i).nodeName.toString());
				attr[i].value = String(attrs.item(i).nodeValue.toString());
				attr[i].specified = true;
			}
		...
		get id() { return String(this.getAttribute("id") || ""); },
		...
		insertAdjacentHTML: function(where,html){
			...
			var node = this._dom.ownerDocument.importNode(
				new DOMDocument( new java.io.ByteArrayInputStream(
					(new java.lang.String("<wrap>"+html.trim()+"</wrap>"))
						.getBytes("UTF8"))).documentElement.firstChild._dom, true);
			
			switch(where)
			{
				case 'beforeBegin':
				{
					this._dom.parentNode.appendChild(node);
					this._dom.parentNode.insertBefore(node, this._dom);
					break;
				}
				case 'afterBegin':
				{
					var hasChildren = this._dom.firstChild;
					this._dom.appendChild(node);					
					if (hasChildren)
					{
						this._dom.insertBefore(node, this._dom.firstChild);
					}
					break;
				}
				case 'beforeEnd':
				...
				case 'afterEnd':
				{
					...
					if (this._dom.nextSibling)
					{
	 	     			this._dom.parentNode.insertBefore(node, this._dom.nextSibling);
					}
					break;
				}
			}
